...
2. **Fill in your test environment values** in `.env.test`. Set the fields required by `ESPRMNeoBase.configure` / `ESPRMNeoBaseConfig`: `baseUrl`, `userApiBase` (include stage, e.g. `…/prod`), `awsRegion`, and `iotEndpoint`. Keep real deployment details and test accounts only in `.env.test` (do not commit it).
