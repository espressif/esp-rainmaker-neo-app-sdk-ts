...
import { StorageKeys, AssumeRoleErrorMessages } from "../../src/utils/constants";
...
describe("ESPRMNeoUser MQTT Methods", () => {
  ...
  describe("assumeRole", () => {
    ...
    it("should POST services and node_id for per-node service credentials", async () => {
      const mockResponse = {
        access_key: "node-access-key",
        secret_key: "node-secret-key",
        session_token: "node-session-token",
      };
      mockAPIManager.post.mockResolvedValue(mockResponse);
      ...
      const result = await userInstance.assumeRole(
        ...
        "test-secret-key",
        "test-session-token",
        { services: ["s3", "kvs"], nodeId: "node-1" }
      );

      const [endpoint, body] = mockAPIManager.post.mock.calls[0];
      expect(endpoint).toBe("/v1/assumed-roles");
      expect(body).toEqual({
        access_key: "test-access-key",
        secret_key: "test-secret-key",
        session_token: "test-session-token",
    ...
    it("should throw when services is set without nodeId", async () => {
      await expect(
        userInstance.assumeRole("key", "secret", "token", {
          services: ["s3"],
        })
      ).rejects.toThrow(AssumeRoleErrorMessages.NODE_ID_REQUIRED);
      expect(mockAPIManager.post).not.toHaveBeenCalled();
    });
    ...
    it("should throw error when response is missing required fields", async () => {
      ...
      await expect(
        userInstance.assumeRole("key", "secret", "token")
      ).rejects.toThrow(AssumeRoleErrorMessages.INVALID_RESPONSE);
    });
