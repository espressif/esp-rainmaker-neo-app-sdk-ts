...
// Resolve first defined from CDK/unifiedcdf-style names or RMNEO_* (for esp-rainmaker-home backend)
...
function envRequired(...keys: string[]): string {
  ...
  if (!v) {
    throw new Error(
      `Missing required env: one of ${keys.join(", ")}. Use same values as unifiedcdf-integration.ts or .env.test.example.`
    );
  }
  return v;
}

/** Get SDK config from env. Accepts CDK-style (ApiGatewayUrl, UserPoolId, ...) or RMNEO_* so deployment details from unifiedcdf-integration can be used. */
