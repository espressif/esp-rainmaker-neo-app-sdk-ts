...
async function main() {
  ...
  for (const specCfg of specs) {
    ...
    for (const op of specCfg.operations ?? []) {
      ...
      // Export response schemas the spec defines inline on the operation
      // (no named component to list in exportedSchemas) under a stable
      // bundle name, so `validated()` call sites keep their historical names.
      for (const [status, as] of Object.entries(op.responseSchemas ?? {})) {
        const respSchema =
          spec.paths?.[op.path]?.[op.method]?.responses?.[status]?.content?.[
            "application/json"
          ]?.schema;
        if (!respSchema) {
          problems.push(
            `responseSchemas["${status}"] but no JSON response schema in ${specCfg.specFile}: ${key}`
          );
          continue;
        }
        if (schemaBundle[as]) {
        ...
        schemaBundle[as] = openapiToJsonSchema(respSchema);
        schemaFingerprints[specCfg.specFile][as] = fingerprintSchema(respSchema);
        console.log(`[schema-gen]   response  ${key} ${status} → ${as}`);
      }
    }
  }
